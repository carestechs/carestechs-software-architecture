# Stack Profile: .NET + Angular Modular Monolith with AI Agent

**Status:** Active
**Assumes:** .NET 10+, Angular 20+, PostgreSQL (with pgvector), EF Core, Tailwind CSS 4+, Microsoft.Extensions.AI

## Overview

A curated set of ADRs for building a modular monolith backend with an Angular SPA frontend, extended with AI agent capabilities. This stack builds on the base `.NET + Angular Modular Monolith` profile and adds ADRs for LLM integration, tool calling, RAG, and conversation management. ADRs are categorized by how essential they are to the stack's coherence.
