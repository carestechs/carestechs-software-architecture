...
- Publishing to npm gives automatic versioning, changelogs via `npm version`, and dependency resolution for consumers who want to pin a specific version.
...
- `package.json` MUST declare `"files"` to include only `dist/` and essential files — NEVER publish `src/`, `tests/`, or config files.
