...
- Backend Dockerfiles MUST use a slim, alpine, or runtime-only base image for the final stage (e.g., `python:3.12-slim`, `mcr.microsoft.com/dotnet/aspnet:<version>-alpine` — the runtime image, not the SDK). NEVER use full SDK or build images as the final stage.
