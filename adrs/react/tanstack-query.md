...
- Alternatives considered: `useEffect` + `useState` (rejected — leads to boilerplate, race conditions, and inconsistent loading/error patterns), Redux Toolkit Query (rejected — heavier, Redux dependency unnecessary for this use case), SWR (rejected — fewer features, less active development than TanStack Query).
...
- Loading and error states MUST be handled using the `isLoading`, `isError`, and `error` properties from query hooks, or via React Suspense boundaries.
