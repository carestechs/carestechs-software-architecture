...
## Decision
The AI agent is a dedicated feature module (`src/app/modules/ai/`) that follows all modular monolith conventions: its own package directory, its own SQLAlchemy models, its own routers/services/schemas, and a dedicated Alembic migration branch. The AI module accesses other modules exclusively through shared contract interfaces.
...
- Migrations for AI-owned tables MUST be in their own Alembic branch or clearly prefixed.
