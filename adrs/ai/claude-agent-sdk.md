...
All AI-powered analysis uses the Claude Agent SDK (`@anthropic-ai/agent-sdk`) for TypeScript. The SDK is used directly — no abstraction layer. AI interaction is encapsulated in a single orchestrator component; no other component calls the SDK directly. AI responses are treated as untrusted input and validated before use.

## Rationale

- The Claude Agent SDK is purpose-built for building tool-using agents with Claude models. It handles conversation management, tool registration, structured output, and streaming natively. Using it directly avoids a premature abstraction layer before the usage patterns are understood.
...
- Token usage MUST be tracked per review session — log input/output token counts for cost awareness.
