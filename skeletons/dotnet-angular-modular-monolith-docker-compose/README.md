...
| Area | ADRs exercised |
|------|----------------|
| Modular monolith: `MyApp.Modules.Catalog` as its own csproj, `MyApp.Contracts` for cross-module interfaces, thin `MyApp.Api` host (`Program.cs` + `Infrastructure/`) with `AddCatalogModule()` | `dotnet/modular-monolith`, `dotnet/thin-api-host` |
| `CatalogDbContext` mapping only catalog entities; per-module EF migration generated with `--context CatalogDbContext` | `dotnet/dbcontext-per-module` |
...
| Cross-module contract shape: `MyApp.Contracts.ICatalogService` + `ProductSummary` resolved by Guid | `dotnet/cross-module-by-id` (contract shape) |
...
| xUnit (v3) per-module test project; `WebApplicationFactory<Program>` against a real PostgreSQL | `dotnet/xunit-per-module-tests` |
...
- **A second module (Identity)** — cross-module communication is shown only as the `Contracts` interface + Guid-reference shape.
...
- The module keeps its internal `Services/ICatalogService`; with a single module, the cross-module
  `MyApp.Contracts.ICatalogService` is implemented by the same class — a second module would consume
  only the contract.
